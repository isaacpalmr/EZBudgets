...
<body>
    ...
    <main>
        ...
        <div id="budget-metadata">
            <div id="budget-title">
                <label>Title:</label>
                <span id="subbudget-title">Loading...</span>
            </div>
        ...
        <div id="tables">
            ...
            <div id="right-side">
                ...
                <div>
                    <input id="save-and-close" type="button" value="Return to Prime Budget" style="margin-left:10px;">
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        //Get the current subudgetid
        const currentBudgetId = <?php echo isset($_GET['subbudget_id']) ? intval($_GET['subbudget_id']) : 0; ?>;
        window.currentBudgetId = currentBudgetId;

        
        //Inherit date and title from main budget
        (async () => {
            try {
                if (!currentSubbudgetId) throw new Error("No subbudget_id provided.");

                const data = await fetchJson(`get_subbudget_details.php?subbudget_id=${currentSubbudgetId}`);
                if (!data || data.error) throw new Error(data?.error || "No data returned");

                // Store inherited values
                window.subbudgetDetails = {
                ...
                // Fill in title
                document.getElementById("subbudget-title").textContent = `${data.budget_name} - ${data.granted_to} Subaward`;

                //Values can be set like
                // const x = new Date(window.subbudgetDetails.startDate);
            } catch (err) {
                console.error("Error loading subbudget details:", err);
                document.querySelector("#budget-title input").value = "Error loading title";
            }
        })();
        ...
        const budgetStartDate = new Date(window.subbudgetDetails.startDate);
        const budgetEndDate = new Date(window.subbudgetDetails.endDate);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const personnelSelect = document.querySelector('#personnel-type-select'); // adjust selector
            if (personnelSelect) {
                personnelSelect.value = ''; // clears selection
            }
        });
